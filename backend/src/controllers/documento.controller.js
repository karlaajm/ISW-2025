...
export async function createDocumento(req, res) {
	try {
		...
		const [documento, err] = await createDocumento({
			nombre,
			ID_CEE,
			fecha_subida,
...
export async function updateDocumento(req, res) {
	try {
		...
		if (err) return handleErrorClient(res, 400, "Error actualizando documento", err);
...
export async function deleteDocumento(req, res) {
	try {
		...
		if (err) return handleErrorClient(res, 404, "Error eliminando documento", err);
