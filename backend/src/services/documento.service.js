...
export async function createDocumento({ nombre, ID_CEE, archivoBase64 }) {
    try {
        const repo = AppDataSource.getRepository(Documento);
        const documento = repo.create({ nombre, miembroCEE: { id: ID_CEE }, archivoBase64 });
...
export async function updateDocumento(id, { nombre, ID_CEE, archivoBase64 }) {
    try {
        const repo = AppDataSource.getRepository(Documento);
